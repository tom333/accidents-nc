# Accidents Project - AI Coding Agent Instructions

## Project Overview
Traffic accident prediction model for New Caledonia (département 988). Uses geospatial data, OSM road networks, and French government accident datasets to build a binary classifier predicting accident risk by location and time.

## Architecture & Data Flow

### Primary Workflow (accident_fetch_data.py)
This is a **Marimo reactive notebook** (not standard Python) with mixed Python and SQL cells:

1. **Data Ingestion (SQL cells)**: Fetches 5 years of accident data from data.gouv.fr CSV URLs using DuckDB's `read_csv()` with `union_by_name=true`
2. **Feature Engineering (SQL)**: Extracts datetime features from French date format (`jour/mois/an hrmn`), filters for New Caledonia (`dep='988'`), creates target=1 for accidents
3. **Road Network Generation**: Uses OSMnx to fetch driving networks for 30+ New Caledonia communes, buffers roads by 200m, creates spatial grid (0.02° resolution)
4. **Negative Sampling**: Generates 5x synthetic non-accident samples by randomly sampling accident features, sets target=0
5. **ML Pipeline**: RandomForestClassifier with RandomizedSearchCV, optimized for AUC-ROC

### Key Technical Patterns

**Geospatial Stack**:
- GeoDataFrames in EPSG:4326 (WGS84), reproject to EPSG:3857 (Web Mercator) for metric buffers
- OSMnx downloads: wrap in try/except, cache results to `routes.nc` GeoJSON
- Spatial joins use `gpd.sjoin()` with `predicate="intersects"`

**Data Sources**:
- Caractéristiques CSV: accident metadata (date, location, conditions)
- Usagers CSV: victim-level data (joined on `Num_Acc`)
- Coordinate cleaning: `trim()` and `.replace(',', '.')` for French number format
...
# Launch Marimo notebook server
marimo edit accident_fetch_data.py
```

**Do NOT** run `python accident_fetch_data.py` directly - it's a Marimo notebook, not a script.

### Adding Dependencies
Use `uv add <package>` (project uses uv, not pip). Current stack:
- marimo: reactive notebook framework
- duckdb: in-memory SQL for CSV processing
- polars: dataframe operations
- osmnx, geopandas: geospatial analysis
- sklearn: ML training

## Project-Specific Conventions

**Column Naming**: French dataset uses lowercase inconsistently (`Num_Acc`, `dep`, `lat`, `long`, `atm`, `jour`, `mois`, `an`, `hrmn`)

**Datetime Parsing**: Use `strptime(concat(...), '%d/%m/%Y %H:%M:%S')` pattern for French dates

**Spatial Operations**: Always check CRS before operations:
- Use `.to_crs(epsg=3857)` before `.buffer()` with meters
- Convert back to EPSG:4326 for final output

**File Caching**: Check `os.path.exists()` before expensive OSMnx downloads; cache to GeoJSON

**Negative Sampling Strategy**: Sample from accident feature distributions (not uniform random) to match marginal distributions

## Known Issues & Gotchas

- OSMnx may timeout for large regions → fetch commune-by-commune, handle exceptions
- DuckDB SQL cells require semicolons only for multi-statement queries
- `routes.nc` file grows large (~50MB); git-ignore recommended
- Random state fixed at 42 for reproducibility
- Model selection uses 5-fold CV with AUC-ROC scorer (imbalanced dataset)

## Entry Points

- **accident_fetch_data.py**: Main analysis notebook (run with Marimo)
- **main.py**: Placeholder CLI (currently prints "Hello from accidents!")
- **pyproject.toml**: Dependency management (Python 3.13+)
