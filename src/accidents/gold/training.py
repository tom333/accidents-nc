...
def _upload_to_s3(local_path: Path, s3_key: str) -> bool:
    try:
        ...
        print(f"Upload to S3: s3://{S3_BUCKET}/{s3_key}")
        return True
    except Exception as exc:
        print(f"Upload S3 error: {exc}")
        return False
...
def _prepare_data():
    ...
    if train_df.empty or test_df.empty:
        raise ValueError("gold.train/test are empty")
...
class TabNetServingModel:
    def __init__(self, trained_model: TabNetClassifier, init_params: dict[str, Any]):
...
class EmbeddingNeuralNetWrapper:
    ...
    def predict_proba(self, X):
        if not hasattr(X, "values"):
            raise ValueError("Model expects a DataFrame input")
...
def run_training() -> dict[str, Any]:
    ...
    if mlflow.active_run():
        mlflow.end_run()
    mlflow.start_run(run_name="gold-training")
    mlflow.log_params(
        {
    ...
    trials = int(os.getenv("OPTUNA_TRIALS", "30"))
    ...
    study_catboost.optimize(objective_catboost, n_trials=trials, show_progress_bar=False)
    ...
    study_lgbm.optimize(objective_lgbm, n_trials=trials, show_progress_bar=False)
    ...
    study_xgb.optimize(objective_xgb, n_trials=trials, show_progress_bar=False)
        ...
        tuning_results.append(
            {
                ...
                "Time": "n/a" if train_time is None else f"{train_time:.0f}",
    ...
    _log_metrics("LogisticRegression", logistic_pipeline, None, {"C": 0.5})

    try:
        tabnet_cat_cols = [
            col
            for col in ["atm", "road_type", "is_weekend", "is_holiday", "school_holidays"]
        ...
        print(f"TabNet training skipped: {exc}")
    ...
    try:
        ...
        cat_train_matrix = (
            _encode_categorical_matrix(X_train[neural_cat_cols], cat_mappings)
            if neural_cat_cols
            else np.zeros((len(X_train), 0), dtype=np.int64)
        )
        cat_test_matrix = (
            _encode_categorical_matrix(X_test[neural_cat_cols], cat_mappings)
            if neural_cat_cols
            else np.zeros((len(X_test), 0), dtype=np.int64)
        )
        ...
        wrapper = EmbeddingNeuralNetWrapper(
            model, neural_cat_cols, cont_cols_nn, cat_mappings, scaler
        )
        _log_metrics(
            "NeuralEmbedding",
            wrapper,
            None,
            {"cat_features": len(neural_cat_cols), "cont_features": len(cont_cols_nn)},
        )
        non_registerable.add("NeuralEmbedding")
    except Exception as exc:
        print(f"Neural embedding training skipped: {exc}")
    ...
    if best_model_name not in non_registerable:
        ...
        print(f"Best model {best_model_name} not registered")
    ...
    return {
        ...
        "best_metrics": {k: float(v) for k, v in best_metrics.items() if k != "training_time_seconds"},
        "leaders": results_df.head(5)[["Model", "Recall"]].to_dict(orient="records"),
    }
