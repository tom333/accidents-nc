...
from ..utils.spatial import buffer_routes, create_spatial_grid, spatial_exclusion_filter
from ..utils.temporal import compute_temporal_features
...
def generate_negative_samples(accidents: pl.DataFrame, routes_grid: pd.DataFrame) -> pl.DataFrame:
    ...
    accident_coords = accidents.select(['latitude', 'longitude']).to_pandas().values
    ...
    accident_datetimes = accidents['datetime'].to_pandas()
    risk_timestamps = np.random.choice(accident_datetimes, size=n_risk_hours, replace=True)
    
    # Timestamps aléatoires
    min_date = accident_datetimes.min()
    max_date = accident_datetimes.max()
    random_timestamps = pd.to_datetime(
        np.random.randint(min_date.value, max_date.value, size=n_random_hours),
        unit='ns'
    )
    ...
    sampled_grid = safe_grid.sample(n=n_samples, replace=True, random_state=42).reset_index(drop=True)
    
    # Créer DataFrame
    negatives = pl.DataFrame({
        'datetime': all_timestamps,
        'latitude': sampled_grid['latitude'].values,
        'longitude': sampled_grid['longitude'].values,
        'atm': np.random.choice(accidents['atm'].to_pandas().dropna(), size=n_samples),
        'hour': pd.to_datetime(all_timestamps).hour,
        'dayofweek': pd.to_datetime(all_timestamps).dayofweek,
        'month': pd.to_datetime(all_timestamps).month,
        'target': np.zeros(n_samples, dtype=int)
    })
    
    print(f"✅ Négatifs générés: {len(negatives)} ({n_risk_hours} risque + {n_random_hours} aléatoire)")
    return negatives
