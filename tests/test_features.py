...
class TestFeatureEngineering:
    ...
    def test_feature_count(self):
        ...
        sample_data["is_rush_morning"] = (
            (sample_data["hour"] >= 7) & (sample_data["hour"] <= 9)
        ).astype(int)
        sample_data["is_rush_evening"] = (
            (sample_data["hour"] >= 17) & (sample_data["hour"] <= 19)
        ).astype(int)
        sample_data["is_night"] = ((sample_data["hour"] >= 22) | (sample_data["hour"] <= 6)).astype(
            int
        )
        ...
        # Vérifications
        assert len(sample_data.columns) == 24, (
            f"Attendu 24 features, obtenu {len(sample_data.columns)}"
        )
    ...
    def test_temporal_indicators(self):
        ...
        # Weekend
        assert 5 >= 5, "Samedi (5) devrait être weekend"
        assert 6 >= 5, "Dimanche (6) devrait être weekend"
        assert not (2 >= 5), "Mercredi (2) ne devrait pas être weekend"
...
class TestPredictionOutput:
    ...
    def test_prediction_shape(self):
        ...
        assert len(predictions) == expected_predictions, (
            f"Attendu {expected_predictions} prédictions"
        )
        assert set(predictions["hour"].unique()) == set(range(24)), (
            "Toutes les heures devraient être présentes"
        )
