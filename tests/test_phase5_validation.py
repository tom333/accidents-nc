...
à la structure actuelle (dagster_pipeline/).
"""
...
def test_tiltfile_updated():
    ...
    with open("Tiltfile", 'r') as f:
        ...
        if "./src/accidents" not in content:
            ...
            return False
        if "./dagster_pipeline" not in content:
            print("   ❌ Tiltfile ne référence pas ./dagster_pipeline")
            return False
...
def test_dockerfile_dagster_updated():
    ...
    with open("Dockerfile.dagster", 'r') as f:
        content = f.read()
        
        if "/app/src" not in content:
            print("   ❌ Dockerfile.dagster ne copie pas /app/src")
            return False
        if "/app/dagster_pipeline" not in content:
            print("   ❌ Dockerfile.dagster ne copie pas /app/dagster_pipeline")
            return False
...
def test_deployment_dagster_updated():
    ...
    with open("k8s/dagster/deployment-dagster-user.yaml", 'r') as f:
        content = f.read()
        
        if "dagster_pipeline.definitions" not in content:
            print("   ❌ deployment ne référence pas dagster_pipeline.definitions")
            return False
...
def test_streamlit_k8s_manifests():
    ...
    with open("k8s/streamlit/deployment.yaml", 'r') as f:
        content = f.read()
